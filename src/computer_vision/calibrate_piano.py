...
def main():
  ...
  print("when the mask lines up with the piano, press c to confirm or p to go back")
  print("Press q to quit and p to capture frame.")
  ...
  while True:
    ...
    # If we have a captured image, process it with confirmation options
    if captured_image is not None:
      poses = pose_tracker.get_marker_poses(captured_image, camera_matrix, dist_coeffs, MARKER_SIZE*IN_TO_METERS)
      marker_list_2d = polygon_detector.get_marker_polygon(MARKER_IDS, poses, captured_image, MARKER_SIZE * IN_TO_METERS)
      if not np.array_equal(marker_list_2d, [0,0,0,0]):
        
        warped = finger_tracker.transform_image_to_birdseye(captured_image, marker_list_2d)
    ...
    # start_time = time.time()
    # median3 = cv.medianBlur(gray, 3)
    # median5 = cv.medianBlur(gray, 5)
    # median_blur_time = time.time() - start_time
    # print(f"Median blur (3x3 and 5x5): {median_blur_time:.4f} seconds")

    start_time = time.time()
    # Ridge detection filter - create with proper parameters
    ...
    total_processing_time = (adaptive_threshold_time + 
    ...
    if SAVE_PICTURES: 
      ...
      cv.imwrite(os.path.join(output_dir, "10_3x3median.jpg"), median3)
      cv.imwrite(os.path.join(output_dir, "10_3x3median.jpg"), median5)
      ...
      # Save key masks
      cv.imwrite(os.path.join(output_dir, "16_binary_filled.jpg"), key_masks['binary_filled'])
      cv.imwrite(os.path.join(output_dir, "17_watershed_mask.jpg"), key_masks['watershed_mask'])
      cv.imwrite(os.path.join(output_dir, "18_contour_mask.jpg"), key_masks['contour_mask'])
      cv.imwrite(os.path.join(output_dir, "19_template_mask.jpg"), key_masks['template_mask'])
      cv.imwrite(os.path.join(output_dir, "20_combined_mask.jpg"), key_masks['combined_mask'])
      cv.imwrite(os.path.join(output_dir, "21_final_mask.jpg"), key_masks['final_mask'])
      cv.imwrite(os.path.join(output_dir, "22_horizontal_lines.jpg"), key_masks['horizontal_lines'])
      cv.imwrite(os.path.join(output_dir, "23_vertical_lines.jpg"), key_masks['vertical_lines'])
      
      # Save adaptive threshold results
      for name, mask in key_masks['adaptive_masks'].items():
        cv.imwrite(os.path.join(output_dir, f"24_{name}.jpg"), mask)
